...
\newcommand{\GroupThree}{
    ...
    \item 维纳霍夫方程：\blank{$ r_{xd}(k) = r_{xx}(k) * h(k) $} \\[4mm] % 增加方程后的间距
    其中 $k$ 的取值范围取决于滤波器类型：
    \blank{$ k = \begin{cases} 0 \sim M - 1 & \text{FIR} \\ 0 \sim +\infty & \text{因果 IIR} \\ -\infty \sim +\infty & \text{非因果 IIR} \end{cases} $} \\[4mm] % 增加公式后的间距
    ...
    % 【修正】
    \item 为实现自适应滤波器作用，\blank{输入信号 $ x(n) $ 和 期望信号 $ d(n) $ 应该相关}
}
